...
def insertBLOB(fname, lname, phoneno, email, loc, emergcont, pic, vol):
    sqliteConnection = sqlite3.connect('calamitigator.db')
    cursor = sqliteConnection.cursor()
    print("Connected to SQLite")
    empPhoto = convertToBinaryData(pic)

    cursor.execute("""
   INSERT OR IGNORE INTO population (Firstname, Lastname, Phonenumber, Email, location, Emergencycontact, ProfilePicture, Voluntering ) VALUES (?,?,?,?,?,?,?,?)""", (fname, lname, phoneno, email, loc, emergcont, empPhoto, vol))
...
def messagequery():
    ...
    latitute = -37.8076
    longitute = 144.9216
    cursor.execute("SELECT location, Phonenumber FROM population")
    loc = cursor.fetchall()
    c = []
    longitutelist = {}
    latitutelist = {}
...
def volunteeringquery():
    sqliteConnection = sqlite3.connect('calamitigator.db')
    cursor = sqliteConnection.cursor()
    cursor.execute("SELECT Firstname, Lastname, Phonenumber, location, Voluntering FROM population")
    lis = cursor.fetchall()
    contactlist = {}
    firstnamelist = {}
    emailids = {}
    ...
    for j in range(len(c)):
        if(j%2==0):
            ...
            longitutelist[r] = c[j]
            r+=1
    latitute = float(latitutelist[0])
    longitute = float(longitutelist[0])
    volunteeredlist = {}
    affectedlist = {}
    c=0
